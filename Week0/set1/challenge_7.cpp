...
vector<uint8_t> base64_decode(const string& characters)
{
    vector<uint8_t> out;
...
void KeyExpansion(const uint8_t* key, uint8_t* RoundKey) {
    ...
    for(int i = 4; i < 44; ++i) {
        int k = (i - 1) * 4;
        uint8_t tempa[4] = {RoundKey[k+0], RoundKey[k+1], RoundKey[k+2], RoundKey[k+3]};
...
void InvCipher(uint8_t* state_arr, const uint8_t* RoundKey) {
    uint8_t state[4][4];
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] = state_arr[i * 4 + j];
        }
    }
...
int main()
{
    ...
    auto ciphertext = base64_decode(file_str);
    string key_str = "YELLOW SUBMARINE";
    
    uint8_t RoundKey[176];
    KeyExpansion((const uint8_t*)key_str.c_str(), RoundKey);

    for (size_t i = 0; i + 16 <= ciphertext.size(); i += 16) {
        InvCipher(&ciphertext[i], RoundKey);
    }
